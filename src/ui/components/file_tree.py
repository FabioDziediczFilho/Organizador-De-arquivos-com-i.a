...
class FileTreeWidget(QTreeWidget):
    ...
    def _on_selection_changed(self):
        ...
        try:
            ...
            # Em caso de erro ao obter itens selecionados (objetos C++ deletados)
            try:
        ...
        if not selected_items:
            # Seleção vazia, limpa o preview emitindo None e lista vazia
            try:
                self.file_selected.emit(None)
                self.files_selected.emit([])
            except Exception as e:
                print(f"[ERROR] Erro ao emitir signals para seleção vazia: {e}")
            return

        # Obter o primeiro item selecionado com segurança
        file_item = None
        try:
            first_item = selected_items[0]
            file_item = first_item.data(0, Qt.UserRole)
        except RuntimeError:
            # Item já foi deletado no C++ (comum durante self.clear())
            try:
                self.file_selected.emit(None)
                self.files_selected.emit([])
            except Exception:
            ...
            print(f"[ERROR] Erro ao obter dados do primeiro item: {e}")

        # Emitir file_selected se houver apenas 1 item selecionado
        if len(selected_items) == 1:
            print("[DEBUG] Emitindo file_selected")
            try:
                self.file_selected.emit(file_item)
            except Exception as e:
                print(f"[ERROR] Erro ao emitir file_selected: {e}")
        else:
            # Se múltiplos itens estiverem selecionados, limpa o preview de item individual
            try:
                self.file_selected.emit(None)
            except Exception:
                pass

        # Obter todos os file_items com segurança
        file_items = []
